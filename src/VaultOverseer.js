...
export async function main(ns) {
  ...
  if (flag.h === "undefined") { flag.h = flag.v; }
  if (ns.getServerMaxRam(flag["h"]) === 0 || ns.getServerMoneyAvailable(flag["v"]) === 0) { ns.exit(); }
  ...
  while (true) {
    ...
    if (currentNbrHackers / 25 > 0) {//hacker compensation
      ...
      do {
        await ns.sleep(1);
        if (ns.getServerUsedRam(flag["h"]) === 0) { breaker = Infinity; break; }
      }
...
/** @param {NS} ns */
export default async function vOe(ns, ...argument) {//VaultOverseerExported
