...
export async function uniquePathsInAGridI(ns, contractName, serverName, outsourceArray = []) {
	let contractData;
	if (outsourceArray.length !== 0) { contractData = outsourceArray; }
...
export async function uniquePathsInAGridII(ns, contractName, serverName) {
	...
	if ((contractData[0][0] || contractData[n - 1][m - 1]) === 1) { return 0 }//Base case where there are obstacles on the start or the finish point

	//Surrounding the map with "walls"
	contractData.unshift(Array(m).fill(1));//Up-most wall
	for (let i of contractData) { i.unshift(1) }//Left-most wall

	n = contractData.length
	m = contractData[0].length

	//Changing 1 to 0 and vice versa
	for (let i in contractData) for (let j in contractData[i]) { contractData[i][j] = 1 - contractData[i][j] }
...
export async function shortestPathInAGridSelfMade(ns, contractName, serverName) {
	...
	let n = contractData.length
	let m = contractData[0].length
	if ((contractData[0][0] || contractData[n - 1][m - 1]) === 1) { return "" }//Base case where there are obstacles on the start or the finish point

	//Surrounding the map with "walls"
	contractData.unshift(Array(m).fill(1));//Up-most wall
	for (let i of contractData) { i.unshift(1) }//Left-most wall
	contractData.push(Array(m).fill(1));//Down-most wall
	for (let i of contractData) { i.push(1) }//Right-most wall
	...
	while (calculationsBuffer < 10) {
		...
		for (let i = 1; i < n - 1; i++) {
			for (let j = 1 + Math.floor(1 / i); j < m - 1; j++) {
				...
				if (dp[i][j].length !== 1) { dp[i][j].shift() }//If there are some solutions, remove the default "" answer
				dp[i][j] = dp[i][j].sort((a, b) => { return a.length - b.length })[0];
			}
		}
		dp[1][1] = "O";
		ns.print(dp);
		...
		counter++;
		//await ns.sleep();
		if (dp[n - 2][m - 2] !== "") { calculationsBuffer++ }//Once the terminus starts being reached by the calculations, we could end the loop, but I want it to loop a few more times to receive a few stragglers
		if (counter > n * m) { break; }//If after "Area" loops, no answer has been found, it's likely that there is no solution because of a wall
	}
	ns.print(dp)
...
export async function deathToParenthesesBroken(ns, contractName, serverName) {
	...
	function parenthesesMatcher(arrayToTreatTemp) {
		...
		for (let i in openers) {
			...
			if (firstIndexesOpeners[i] === -1) { firstIndexesOpeners[i] = workingArray.length - 1 }
			if (lastIndexesClosers[i] === -1) { lastIndexesClosers[i] = 0 }
		}
	...
	*/
	function parenthesesTrimmer(array) {
		while (closers.includes(array[0])) { array.shift() }
		while (openers.includes(array[array.length - 1])) { array.pop() }
		return array
	}
	function parenthesesWeddinger(workingArray) {
		console.log(workingArray);
		let nonParentheses = Array(workingArray.length).fill("");
		for (let i in workingArray) if (!(closers.includes(workingArray[i]) || openers.includes(workingArray[i]))) { nonParentheses[i] = workingArray[i] }
		...
		for (let i = 0; i < dp[0][0]; i++) for (let b in dp[i]) {
			...
			for (let j = i + 1; j < dp.length; j++) for (let c in dp[j]) {
				if ((((j < dp[i][b]) && (c < dp[i][b])) || ((j > dp[i][b]) && (c > dp[i][b]))) && (tempAnswerArray[j] === tempAnswerArray[c])) {
					for (let z = j; z < c; z++) { tempAnswerArray[z] = j }
		...
		for (let i in unprocessedAnswer) {//Reintroduce the non-parentheses
			...
			for (let b = 0; b < unprocessedAnswer[i].length + nonParentheses.length; b++) {
				let addition = "";
				if (typeof unprocessedAnswer[i][b] === "string") { addition = unprocessedAnswer[i][b] }
...
export async function findAllValidMathExpressionsHomemade(ns, contractName, serverName) {
	...
	let operators = ["+", "-", "*"];
	if (parseInt(baseString) < targetNumber) { return [] }
	let result = [];
	/** @param {Number} n 
	 *  @param {String} string
	 *  @return {Array}*/
	function recursiveFunction(n, string) {
		if (n >= string.length - 1) {//Reached the end of the string
			if (isEqualToTarget(string)) { result.push(string); return; }
		}
	...
	function isEqualToTarget(string) {
		for (let i in operators) { string.replaceAll(operators[i], " " + operators[i] + " ") }
