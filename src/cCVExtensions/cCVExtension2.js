...
export async function mergeOverlappingIntervals(ns, contractName, serverName) {
	...
	contractData.sort(function (a, b) { return a[0] - b[0] });
	...
	while (!goodBehavior) {
		...
		for (let i in contractData) {
			for (let b in contractData) {
				if (i === b) { continue; }
				if ((contractData[b][0] >= contractData[i][0]) && (contractData[b][1] <= contractData[i][1])) { contractData.splice(b, 1); goodBehavior = false; break; }//b is completely within i
...
export async function generateIPAdressesSelfMade(ns, contractName, serverName) {
	...
	let answer = [];
	let contractArray = [];
	for (let i = 0; i < contractData.length; i++) { contractArray.push(contractData[i]); }
	/** @param {Number} n
	 *  @param {Any[]} previous */
	function IPmaker(n, previous) {//n = position of the new dot, previous = passed array
		...
		if ((previous[n] !== "0") && (previous[n - 1] !== ".") && (previous[n] !== ".") && (n !== 0)) {
			...
			if (!nope) { answer.push(string); }
			if (!megaNope) { IPmaker(n + 1, newer); }
		}
		else if (previous[n] === "0" && (previous[n - 1] !== ".") && (n !== 0)) {
			let newer = previous.toSpliced(n, 0, ".");
			newer.splice(n + 2, 0, ".");
			if (newer[newer.length - 1] === ".") { newer.pop() }
			let string = "";
			for (let i = 0; i < newer.length; i++) { string += newer[i] }
			let nope = false;
			let megaNope = false
...
export async function algoStonksI(ns, contractName = "", serverName = "", researchArray = []) {
	let contractData;
	if (researchArray.length !== 0) { contractData = researchArray }
	else { contractData = ns.codingcontract.getData(contractName, serverName); }
	let memory = [];
	if (contractData.length < 2) { return 0 }
	function trader(n, origin) {
		if (n >= contractData.length) { return 0 }
	...
	return arrayBest.toSorted(function (a, b) { return b - a })[0]
}
