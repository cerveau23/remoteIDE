...
export async function subarrayWithMaximumSum(ns, contractName, serverName) {
    ...
    for (let i = 1; i < data.length; i++) {
        if ((data[i] < 1) && (data1[data1.length - 1] < 1)) {
            data1[data1.length - 1] += data[i]
        } else {
            data1[data1.length] = data[i]
        }
        max = Math.max(Math.max.apply(null, data1), max);
    }
    while (data1[data1.length - 1] < 1) {
        data1.pop()
    }
    while (data1[0] < 1) {
        data1.shift()
    ...
    for (let i = 1; i < data1.length; i++) {
        if ((data1[i] > -1) && (data2[data2.length - 1] > -1)) {
            data2[data2.length - 1] += data1[i]
        } else {
    ...
    for (let i = 0; i < data2.length; i++) {
        if ((data2[i] > -1) && (data2[i - 1] + data2[i] < 1) && (data2[i + 1] + data2[i] < 1)) {
            data3[data3.length - 1] += data2[i] + data2[i + 1];
            i++
        } else {
    ...
    /**ns.tprint("Answer: " + answer);
     ns.tprint("Max: " + max);*/
    for (let i of [data, data1, data2, data3]) {
        ns.print(i)
    }
    return answer
}
...
export async function totalWaysToSumII(ns, contractName, serverName) {
    ...
    function ways(n, index) {
        ...
        if (n < 0 || index === authorizedG.length) {
            return 0;
        }
        if (memoization[n][index] !== null) {
...
export async function spiralizeMatrix(ns, contractName, serverName) {
    ...
    let lengthMax;
    let answer = [];
    if (!Array.isArray(contractData[0])) {
        lengthMax = contractData.length
    } else {
        lengthMax = contractData.length * contractData[0].length
    }
...
export async function arrayJumpingGame(ns, contractName, serverName) {
    ...
    function jumping(n, jump) {
        ...
        if (memoization[n][jump] !== undefined) {
            return memoization[n][jump]
        }
    ...
    function reverseJumping(n) {
        ...
        for (let i = 0; i < n; i++) {
            if (i + contractData[i] >= n) {
                ...
                if (memoization[i] === undefined) {
                    memoization[i] = reverseJumping(i)
                }
                if (memoization[i] !== 0) {
