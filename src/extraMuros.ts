...
export async function main(ns: NS) {
	...
	if ((ns.getServerMaxRam(flag.destination) - ns.getServerUsedRam(flag.destination) >= ns.getScriptRam(flag.script)) && ns.getServer(flag.destination).hasAdminRights) {
		...
		let functionalScriptsList = ns.ls("home", "/functional");
		if (ns.ls(flag.destination).filter((file)=>functionalScriptsList.includes(file)).length !== functionalScriptsList.length)
			{ns.scp(functionalScriptsList, flag.destination)}
